...
class Request {
  request(QueryData: QueryData): Promise<any> {
    ...
    const url =
      (QueryData.host ? BASE_URL[QueryData.host] : environmentData.BaseEnUrl) +
      QueryData.apiUrl;
    return new Promise(function(resolve, reject) {
      ...
      Taro.request(requestData).then(res => {
        ...
        if (statusCode >= 200 && statusCode < 300) {
          if (
            (res && res.data && (res.data as any).err === "Unavailable") ||
            (res && res.data && (res.data as any).status === 401)
          ) {
            if (
              environmentData.CurrentEnvironment == "Android" ||
              environmentData.CurrentEnvironment == "IOS"
            ) {
