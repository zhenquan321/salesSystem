...
router.post("/reply", async ctx => {
  ...
  // 创建回复
  const r = await ReplyService.create(v);
...
router.put("/reply/:id", new Auth(AUTH_ADMIN).m, async ctx => {
  ...
  // 获取分类ID参数
  const id = v.get("path.id");
  await ReplyService.update(id, v);
...
router.get("/reply", async ctx => {
  ...
  let replyList = await ReplyService.list(comment_id);
...
router.get("/reply/:id", async ctx => {
  ...
  // 获取分类ID参数
  const id = v.get("path.id");
  let reply = await ReplyService.detail(id);
