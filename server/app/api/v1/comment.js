...
router.post("/comment", async ctx => {
  ...
  const r = await CommentService.create(v);
...
router.put("/comment/:id", new Auth(AUTH_ADMIN).m, async ctx => {
  ...
  // 获取分类ID参数
  const id = v.get("path.id");
  await CommentService.update(id, v);
...
router.get("/comment", async ctx => {
  ...
  let commentList = await CommentService.list(page);
...
router.get("/comment/:id", async ctx => {
  ...
  // 获取分类ID参数
  const id = v.get("path.id");
  let comment = await CommentService.detail(id);
...
router.get("/comment/target/list", async ctx => {
  let comment = await CommentService.targetComment(ctx.query);
